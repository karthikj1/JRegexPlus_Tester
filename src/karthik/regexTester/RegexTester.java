...
public class RegexTester {
    ...
    public static void main(String[] args) {
        ...
        while (scanner.hasNext())
            try {
                ...
                System.out.println(
                    (SEARCH_FROM_FILE) ? "in file " + search_text_file : inp);
                if (DO_TIMING) {
                ...
                do {
                    ...
                    if (match_success)
                        for (int r = 0; r < matchObj.matchCount(); r++) {
                            ...
                            System.out.print("MATCHED: " + matchObj.
                                groupCount(r) + " groups ");
                            ...
                            if (DO_ORACLE)
                                if (oracle_result) {
                                    ...
                                    if (oracle_result_string.equals(matchObj.
                                        group(r, 0)))
                                        System.out.println("MATCHED oracle");
                                    else
                                        System.out.println(
                                            "DID NOT MATCH oracle");
                                } else {
                                    System.out.println("DID NOT MATCH oracle");
                                    System.out.println(
                                        "oracle groups = " + oracle_num_groups);
                                }
                            System.out.println("Matched string: " + matchObj.
                                group(r, 0)
                            ...
                            for (int i = 1; i <= matchObj.groupCount(r); i++) {
                                ...
                                if (DO_ORACLE && oracle_result) {
                                    ...
                                    System.out.print(
                                        ": " + javaMatcher.group(i) + "\r\n");
                                    if ((javaMatcher.group(i) == null) || (!javaMatcher.
                                        group(i).equals(matchObj.group(r, i))))
                                        System.out.println(
                                            "Group " + i + " DID NOT MATCH ");
                                }
                            }
                        }
                    else {
                ...
                System.out.println("RegexTester Java matcher ERROR: " + ise.
                    getMessage());
    ...
    private static void show_compile_stats(final int NUM_TRIALS,
        final String regex, final String inp)
        ...
        endTime = System.currentTimeMillis();
        System.out.println(
            ", java compiled in " + (endTime - startTime) / NUM_TRIALS);
    }

    private static void show_search_stats(final int NUM_TRIALS,
        final String regex, final String inp) throws MatcherException {
        double startTime, endTime;
        Matcher matchObj;
        ...
        endTime = System.currentTimeMillis();
        System.out.println(
            ", java matched in " + (endTime - startTime) / NUM_TRIALS);
    }
    ...
    private static void show_usage_syntax() {
        ...
        System.out.println(
            "Usage: RegexTester [<test_input_filename>] [-timing] [-oracle] "
            + "[-text <search_text_filename>] [-output <output_file_name>]\r\n");
        System.out.println(
            "[<test_input_filename>]  - Name of input file with test cases. Default is perl_test_cases.txt");
        System.out.println(
            "[-timing] - Processes regex with java.util.regex engine and calculates running time comparison");
        System.out.println(
            "[-output <output_file_name>] - Name of output file for results");
        System.out.println(
            "[-text <text_file_name>] - Name of file containing text to search for all regexes");
        System.out.println(
            "[-all] - Find all matches in search text(default finds just first match)");
        System.out.println("\r\n");
        ...
        while (ctr < args.length) {
            ...
            if (arg.equalsIgnoreCase("-output"))
                if (ctr < args.length) {
                    ...
                    System.out.println(
                        "ERROR: -output must be followed by output file name");
                    show_usage_syntax();
                    System.exit(-1);
                }
            ...
            if (arg.equalsIgnoreCase("-text"))
                if (ctr < args.length) {
                    ...
                    System.out.println(
                        "ERROR: -text must be followed by search text file name");
                    show_usage_syntax();
                    System.exit(-1);
                }
